...
namespace Saab.CBRNSensors.Models
{
    public class AircraftConflict
    {
        ...
        #region Constructors
        public AircraftConflict(string parentAttribute, INETWISEStringCache cache, AttributeGroup data)
        {
        ...
        public AircraftConflict(AttributeHandle parentAttribute, INETWISEStringCache cache, AttributeGroup data)
        {
            ...
            if (!IsInitialized(hAttribute))
            {
                ...
                lock (_nameIdIndex)
                {
                    if (!_nameIdIndex.ContainsKey(hAttribute))
                    {
                        ...
                        _nameIdIndex[hAttribute].Add(strAttributeName + ".Aircraft", Fields.Aircraft);
                        _nameIdIndex[hAttribute].Add(strAttributeName + ".SeverityHigh", Fields.SeverityHigh);
                    }
        ...
        public STS.WISE.ObjectHandle AircraftValue
        {
            get
            {
                AttributeHandle hAttribute = GetFieldHandle(Fields.Aircraft, this.ParentAttribute);

                lock (this.Data)
                {
                    if (this.Data.Handles.ContainsKey(hAttribute))
                    {
                        return this.Data.Handles[hAttribute];
                    }
                }
            ...
            set
            {
                ...
                lock (this.Data)
                {
                    this.Data.Handles[hAttribute] = value;
                }
            }
        }

        public byte SeverityHighValue
        {
            get
            {
                AttributeHandle hAttribute = GetFieldHandle(Fields.SeverityHigh, this.ParentAttribute);
